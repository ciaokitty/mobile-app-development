...
export default function ActivityLog({ onDayPress, onHeadingPress }: { onDayPress?: (date: Date) => void, onHeadingPress?: () => void }) {
  ...
  // Build a list of all days with any data (period, symptoms, mood, weight, text, etc.)
  const dataDates = new Set<string>();
  Object.keys(symptomLogs).forEach(d => dataDates.add(d));
  Object.keys(weightLogs).forEach(d => dataDates.add(d));
  Object.keys(textLogs).forEach(d => dataDates.add(d));
  Object.keys(sexLogs).forEach(d => dataDates.add(d));
  Object.keys(moodLogs).forEach(d => dataDates.add(d));
  // Add all period days
  ...
  const days: Date[] = Array.from(dataDates).map(d => {
